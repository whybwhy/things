...
const nabbo = person('nabbo').setAge(39).setFamily(['bk']).build();
//console.dir(nabbo);
const whybwhy = person('whybwhy').setAge(38).build();
//console.dir(whybwhy);
const test = person("name").build();
//console.log((test));
...
const createCoffee = function(name) {
    let _shot;
    let _milk;
...
const americano = createCoffee("americano").setShot(2).build();
const latte = createCoffee("latte").setShot(2).setMilk(true).build();
latte.mix(americano);
